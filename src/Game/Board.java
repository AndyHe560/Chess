...
public class Board {
    ...
    {
        if(isValidMove(piece, finalX, finalY) && piece.isvalidmove(finalX, finalY))
        {
    ...
    {
        if(piece.getType() == Types.knight) //knights can leap
            return true;

        //pawns only have a path under special circumstances. kings will never have a path
        if(piece.getType() == Types.pawn || piece.getType() == Types.king)
            return true;
    ...
    public boolean isCapture(Piece piece, int finalX, int finalY)
    {
        ...
        {
            if(boardArray[finalX][finalY].getType() == Types.king)
